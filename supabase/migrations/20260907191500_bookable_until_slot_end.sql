-- Keep same-day slots bookable until their end time (e.g. 7–8 PM stays
-- available at 7:15), not only until start time.

drop function if exists public.slot_is_bookable(date, time) cascade;

create function public.slot_is_bookable(p_date date, p_end_time time)
returns boolean
language sql
stable
as $$
  select
    ...
    or (p_date = public.ist_today() and p_end_time > public.ist_now_time());
$$;
