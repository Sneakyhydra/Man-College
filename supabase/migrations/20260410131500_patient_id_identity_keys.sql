...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'patient_queue_entries'
      and column_name = 'system_patient_id'
  ) then
    alter table public.patient_queue_entries
      rename column system_patient_id to patient_id;
  end if;
end $$;
...
declare
  ...
  v_has_upcoming_visit boolean := false;
begin
  ...
  if v_has_upcoming_visit then
    raise exception 'Patient already has an upcoming visit in queue.' using errcode = 'P0001';
  end if;
